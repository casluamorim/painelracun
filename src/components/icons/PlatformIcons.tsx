...
export const getPlatformIcon = (platform: string, size = 20): React.ReactNode => {
  switch (platform) {
    case 'meta':
      return <MetaIcon size={size} className="text-[hsl(214,100%,45%)]" />;
    case 'google':
      return <GoogleIcon size={size} />;
    case 'tiktok':
      return <TikTokIcon size={size} className="text-[hsl(180,76%,48%)]" />;
    default:
      return null;
  }
};
